...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between bg-[--header-bg-color] dark:bg-[--header-bg-color-dark] border-b border-gray-300 dark:border-gray-500 dark:border-opacity-50 py-4 px-3 sm:px-6 md:px-10 lg:px-12 2xl:px-14'>
        ...
        <button className='border border-gray-500 rounded-full px-4 py-2 lg:hidden' onClick={() => setShowSearchBar(!showSearchBar)}>
            {showSearchBar ? <IoClose /> : <IoSearch />}
        </button>
        <form className={`${showSearchBar && window.innerWidth < 1024 ? 'absolute flex items-center left-0 right-0 w-fit mx-auto top-[75px]' : 'relative lg:flex items-center min-w-52 hidden'}`}>
